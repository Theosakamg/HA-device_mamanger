...
export class DmImportView extends LitElement {
  static styles = [
    ...
    css`
      ...
      .upload-zone {
        border: 2px dashed var(--dm-border); border-radius: 8px; padding: 40px;
        text-align: center; cursor: pointer; transition: border-color 0.2s;
        margin-bottom: 24px;
      }
      .upload-zone:hover { border-color: var(--dm-primary); }
      .upload-zone.dragging { border-color: var(--dm-primary); background: rgba(3,169,244,0.05); }
      .file-info { margin: 16px 0; padding: 12px; background: #f5f5f5; border-radius: 4px; display: flex; justify-content: space-between; align-items: center; }
      ...
      .stat-box.created { background: #e8f5e9; color: #2e7d32; }
      .stat-box.updated { background: #e3f2fd; color: #1565c0; }
      .stat-box.skipped { background: #fff3e0; color: #ef6c00; }
      ...
      .log-status { padding: 2px 8px; border-radius: 12px; font-size: 11px; font-weight: 500; }
      .log-status-created { background: #e8f5e9; color: #2e7d32; }
      .log-status-updated { background: #e3f2fd; color: #1565c0; }
      .log-status-skipped { background: #fff3e0; color: #ef6c00; }
      .log-status-error { background: #fce4ec; color: #c62828; }
      .error-panel {
      ...
      .error-panel-body { max-height: 300px; overflow-y: auto; padding: 0; }
      ...
      .error-row-num { font-weight: bold; margin-right: 8px; color: #c62828; }
    `,
  ];
      ...
      ${!this._result
        ? html`
            ...
            >
              ...
              <p style="font-size: 12px; color: var(--dm-text-secondary);">CSV (UTF-8 or Latin-1)</p>
            </div>

            ${this._file
              ? html`
                  <div class="file-info">
                    <span>📄 ${this._file.name} (${(this._file.size / 1024).toFixed(1)} KB)</span>
                    <button class="btn-icon" @click=${() => { this._file = null; }}>✕</button>
                  </div>
                  <button class="btn btn-primary" ?disabled=${this._processing} @click=${this._startImport}>
                    ${this._processing ? i18n.t('import_processing') : i18n.t('import_start')}
                  </button>
                `
              : nothing}
          `
    ...
    return html`
      <div class="result-card">
          ...
          ? html`
              <div class="error-panel">
                <div class="error-panel-header" @click=${() => { this._showErrors = !this._showErrors; }}>
                  <span>⚠️ ${i18n.t('import_errors')} (${r.errors.length})</span>
          ...
          ? html`
              <div class="log-table">
                <table>
                  ...
                  <tbody>
                    ${r.logs.map(
                      (entry: { row: number; status: string; message: string }) => html`
                        <tr>
                          <td>${entry.row}</td>
                          <td><span class="log-status log-status-${entry.status}">${entry.status}</span></td>
                          <td>${entry.message}</td>
                        </tr>
                      `
                    )}
        ...
        <div style="margin-top: 16px;">
          <button class="btn btn-secondary" @click=${this._reset}>${i18n.t('import_csv')}</button>
        </div>
      </div>
    `;
  }
