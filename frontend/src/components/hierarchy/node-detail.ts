...
export class DmNodeDetail extends LitElement {
  static styles = [
    ...
    css`
      :host { display: block; }
      .info-grid { display: grid; grid-template-columns: 120px 1fr; gap: 8px; margin-bottom: 16px; }
      .info-label { font-weight: 500; color: var(--dm-text-secondary); font-size: 13px; }
      .info-value { font-size: 14px; }
      .section-header { display: flex; justify-content: space-between; align-items: center; margin: 16px 0 8px; }
      .children-list { display: grid; gap: 8px; }
      .child-card {
        display: flex; justify-content: space-between; align-items: center;
        padding: 10px 14px; border: 1px solid var(--dm-border); border-radius: 6px;
        cursor: pointer; transition: background 0.15s;
      }
      .child-card:hover { background: rgba(0,0,0,0.02); }
      .child-info { display: flex; align-items: center; gap: 8px; }
      .device-row { padding: 8px 12px; border-bottom: 1px solid var(--dm-border); display: flex; justify-content: space-between; align-items: center; }
      .edit-form { margin: 16px 0; padding: 16px; border: 1px solid var(--dm-border); border-radius: 6px; }
    `,
  ];
  ...
  render() {
    if (!this.node) return html`<div class="empty-state">${i18n.t('no_homes')}</div>`;
    ...
    return html`
      <div class="card-header">
        ...
        <div>
          <button class="btn btn-secondary" @click=${this._startEdit}>✏️ ${i18n.t('edit')}</button>
        </div>
      </div>
        ...
        ? html`
            ...
            <div class="children-list">
              ${this.node.children.map(
                (child) => html`
                  <div class="child-card" @click=${() => this._selectChild(child)}>
                    <div class="child-info">
                      <span>${this._childIcon(child.type)} ${child.name}</span>
                    </div>
                    <span class="badge">${child.deviceCount} ${i18n.t('device_count')}</span>
                  </div>
                `
              )}
            </div>
        ...
        ? html`
            ...
            ${this._loadingDevices
              ...
              : this._devices.length === 0
                ? html`<div class="empty-state">${i18n.t('no_devices_in_room')}</div>`
                : html`
                    <table>
                      <thead>
                        <tr>
                        ...
                        ${this._devices.map(
                          (d) => html`
                            <tr>
                              ...
                              <td>
                                <span class="status-dot ${d.enabled ? 'status-enabled' : 'status-disabled'}"></span>
                                ${d.enabled ? i18n.t('enabled') : i18n.t('disabled')}
                              </td>
                            </tr>
                          `
                        )}
      ...
      <div class="edit-form">
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <input type="text" .value=${this._editSlug}
              @input=${(e: Event) => { this._editSlug = (e.target as HTMLInputElement).value; }} />
          </div>
        </div>
        <div class="form-row">
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input type="text" .value=${this._editImage}
              placeholder="URL or path"
              @input=${(e: Event) => { this._editImage = (e.target as HTMLInputElement).value; }} />
          </div>
        </div>
        <div style="display:flex; gap:8px; margin-top:8px;">
          <button class="btn btn-primary" @click=${this._saveEdit}>${i18n.t('save')}</button>
          <button class="btn btn-secondary" @click=${() => { this._editing = false; }}>${i18n.t('cancel')}</button>
        </div>
      </div>
    `;
  }
  ...
  private async _saveEdit() {
    ...
    try {
      ...
      if (this.node.type === 'home') await this._homeClient.update(this.node.id, data);
      else if (this.node.type === 'level') await this._levelClient.update(this.node.id, data);
      else if (this.node.type === 'room') await this._roomClient.update(this.node.id, data);
      this._editing = false;
  ...
  private _selectChild(child: HierarchyNode) {
    ...
    this.dispatchEvent(new CustomEvent('node-selected', { detail: { node: child }, bubbles: true, composed: true }));
    // Also emit expand-to-node so the tree auto-expands and highlights the child
    this.dispatchEvent(new CustomEvent('expand-to-node', { detail: { node: child }, bubbles: true, composed: true }));
  }
