...
export class DmAppShell extends LitElement {
  static styles = [
    sharedStyles,
    css`
      :host { display: block; min-height: 100vh; background: var(--dm-background); }
      .app-header {
      ...
      .app-title { font-size: 18px; font-weight: 600; color: var(--dm-text); display: flex; align-items: center; gap: 8px; }
      .app-nav { display: flex; gap: 4px; }
      .nav-btn {
        padding: 8px 16px; border: none; background: transparent;
        cursor: pointer; font-size: 14px; border-radius: 4px;
        color: var(--dm-text-secondary); transition: all 0.15s;
      }
      .nav-btn:hover { background: rgba(0,0,0,0.04); color: var(--dm-text); }
      .nav-btn.active { background: rgba(3,169,244,0.1); color: var(--dm-primary); font-weight: 500; }
      .lang-toggle {
        padding: 4px 8px; border: 1px solid var(--dm-border); border-radius: 4px;
  ...
  private _getRouteFromHash(): AppRoute {
    ...
    const validRoutes: AppRoute[] = ['hierarchy', 'devices', 'settings', 'maintenance'];
    return validRoutes.includes(route as AppRoute) ? (route as AppRoute) : 'hierarchy';
  }
  ...
  render() {
    return html`
      <div class="app-header">
        ...
        <nav class="app-nav">
          <button class="nav-btn ${this._route === 'hierarchy' ? 'active' : ''}"
            @click=${() => this._navigate('hierarchy')}>
            🏗️ ${i18n.t('nav_hierarchy')}
          </button>
          <button class="nav-btn ${this._route === 'devices' ? 'active' : ''}"
            @click=${() => this._navigate('devices')}>
            📱 ${i18n.t('nav_devices')}
          </button>
          <button class="nav-btn ${this._route === 'settings' ? 'active' : ''}"
            @click=${() => this._navigate('settings')}>
            ⚙️ ${i18n.t('nav_settings')}
          </button>
          <button class="nav-btn ${this._route === 'maintenance' ? 'active' : ''}"
      ...
      <main class="app-content">
        ${this._route === 'hierarchy' ? html`<dm-hierarchy-view></dm-hierarchy-view>` : ''}
        ${this._route === 'devices' ? html`<dm-device-table></dm-device-table>` : ''}
        ${this._route === 'settings' ? html`<dm-settings-view></dm-settings-view>` : ''}
        ${this._route === 'maintenance' ? html`<dm-maintenance-view></dm-maintenance-view>` : ''}
      </main>
