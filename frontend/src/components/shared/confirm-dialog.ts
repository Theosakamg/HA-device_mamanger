...
export class DmConfirmDialog extends LitElement {
  static styles = [
    sharedStyles,
    css`
      .message { margin: 16px 0; font-size: 16px; line-height: 1.5; }
    `,
  ];
  ...
  render() {
    ...
    return html`
      <div class="modal-overlay" @click=${this._cancel}>
        <div class="modal" @click=${(e: Event) => e.stopPropagation()} style="max-width: 400px;">
          <div class="modal-header">
            <h2>${i18n.t('confirm')}</h2>
          </div>
          <div class="message">
            ${this.message || i18n.t('confirm_delete')}
            ${this.cascade ? html`<br/><strong>${i18n.t('confirm_delete_cascade')}</strong>` : ''}
          </div>
          <div class="modal-actions">
            <button class="btn btn-secondary" @click=${this._cancel}>${i18n.t('cancel')}</button>
            <button class="btn btn-danger" @click=${this._confirm}>${i18n.t('delete')}</button>
          </div>
        </div>
      </div>
    `;
